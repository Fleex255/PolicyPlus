    ...
    public class PolicyLoader
    {
        ...
        public IPolicySource OpenSource()
        {
            ...
            switch (Source)
            {
                ...
                case PolicyLoaderSource.NtUserDat:
                    {
                        ...
                        using (var machHive = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Default))
                        {
                            ...
                            PInvoke.RegLoadKeyW(new IntPtr(int.MinValue + 0x00000002), subkeyName, _mainSourcePath); // HKEY_LOCAL_MACHINE
                ...
                default:
                    {
                        if (System.IO.File.Exists(_mainSourcePath))
                        {
                            // Open a POL file
                            try
                            {
                                using var fPol = new System.IO.FileStream(_mainSourcePath, System.IO.FileMode.Open, System.IO.FileAccess.ReadWrite);
                                _writable = true;
                            }
                            catch (Exception)
                            {
                        ...
                        else
                        {
                            ...
                            try
                            {
                                System.IO.Directory.CreateDirectory(System.IO.Path.GetDirectoryName(_mainSourcePath));
                                var pol = new PolFile();
        ...
        public bool Close() // Whether cleanup was successful
        {
            ...
            var subkeyName = Strings.Split(_mainSourceRegKey.Name, @"\", 2)[1]; // Remove the host hive name
        ...
        public string Save() // Returns human-readable info on what happened
        {
            switch (Source)
            {
                case PolicyLoaderSource.LocalGpo:
                    {
                        var oldPol = System.IO.File.Exists(_mainSourcePath) ? PolFile.Load(_mainSourcePath) : new PolFile();
                        ...
                        {
                            PInvoke.RefreshPolicyEx(!_user, 0U);
        ...
        public string GetCmtxPath() =>
            Source switch
            {
                ...
                PolicyLoaderSource.PolFile or PolicyLoaderSource.NtUserDat => System.IO.Path.ChangeExtension(
                ...
                _ => !string.IsNullOrEmpty(_mainSourcePath)
                    ? System.IO.Path.Combine(System.IO.Path.GetDirectoryName(_mainSourcePath) ?? throw new InvalidOperationException(), "comment.cmtx")
                    : string.Empty
            };
        ...
        private void UpdateGptIni()
        {
            ...
            if (System.IO.File.Exists(_gptIniPath))
            {
                // Alter the existing gpt.ini's Version line and add any necessary other lines
                var lines = System.IO.File.ReadLines(_gptIniPath).ToList();
                using var fGpt = new System.IO.StreamWriter(_gptIniPath, false);
                ...
                foreach (var line in lines)
                {
                    ...
                    {
                        var curVersion = Conversions.ToInteger(Strings.Split(line, "=", 2)[1]);
            ...
            else
            {
                ...
                using var fGpt = new System.IO.StreamWriter(_gptIniPath);
