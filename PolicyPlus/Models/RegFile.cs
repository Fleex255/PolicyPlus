...
namespace PolicyPlus.Models
{
    ...
    public class RegFile : IPolicySource
    {
                ...
                else
                {
                    ...
                    while (true) // Read all the values
                    {
                        ...
                        else if (data.StartsWith("hex"))
                        {
                            ...
                            {
                                foreach (var b in Strings.Split(Strings.Trim(curHexLine).TrimEnd('\\', ','), ",").Where(s => !string.IsNullOrEmpty(s)))
                                {
        ...
        public void Save(StreamWriter writer)
        {
            ...
            foreach (var key in _keys)
            {
                ...
                else
                {
                    ...
                    {
                        var posInRow = 0; // To split hex across lines
        ...
        private RegFileKey GetKey(string name) => _keys.Find(k => k.Name.Equals(name, StringComparison.InvariantCultureIgnoreCase));
