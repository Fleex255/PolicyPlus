    ...
    public partial class PolFile
    {
        private class PolEntryData // Represents one record in a POL file
        {
            ...
            public static PolEntryData FromString(string text, bool expand = false) // Save a UTF-16LE string
            {
                ...
                {
                    var charCode = Strings.AscW(text[x]);
            ...
            public static PolEntryData FromMultiString(string[] strings)
            {
                ...
                {
                    foreach (var charCode in s.Select(Strings.AscW))
                    {
            ...
            public static PolEntryData FromArbitrary(object data, RegistryValueKind kind) =>
                ...
                kind switch
                {
                    RegistryValueKind.String => FromString(Conversions.ToString(data)),
                    RegistryValueKind.DWord => FromDword(Conversions.ToUInteger(data)),
                    RegistryValueKind.ExpandString => FromString(Conversions.ToString(data), true),
                    RegistryValueKind.QWord => FromQword(Conversions.ToULong(data)),
