...
namespace PolicyPlus.Models
{
    public class RegistryPolicyProxy : IPolicySource // Pass operations through to the real Registry
    {
        ...
        public void SetValue(string key, string value, object data, RegistryValueKind dataType)
        {
            data = data switch
            {
                uint => new ReinterpretableDword { Unsigned = Conversions.ToUInteger(data) }.Signed,
                ulong => new ReinterpretableQword { Unsigned = Conversions.ToULong(data) }.Signed,
                _ => data
            };
        ...
        public object GetValue(string key, string value)
        {
            ...
            return data switch
            {
                int => new ReinterpretableDword { Signed = Conversions.ToInteger(data) }.Unsigned,
                long => new ReinterpretableQword { Signed = Conversions.ToLong(data) }.Unsigned,
                _ => data
            };
        }
