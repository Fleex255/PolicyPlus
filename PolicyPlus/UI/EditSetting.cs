                    ...
                    case "decimalTextBox": // Numeric spin box or a plain text box restricted to numbers
                        {
                            ...
                            else
                            {
                                ...
                                text.TextChanged += (sender, e) =>
                                {
                                    ...
                                    var curNum = Conversions.ToInteger(text.Text);
                    ...
                    case "listBox": // Button to launch a grid view editor
                        {
                            ...
                            {
                                if (MyProject.Forms.ListEditor.PresentDialog(listPres.Label, button.Tag,
                                        list.UserProvidesNames) == DialogResult.OK)
                                {
                                    button.Tag = MyProject.Forms.ListEditor.FinalData;
                                }
                            };
                        ...
                        foreach (var kv in PolicyProcessing.GetPolicyOptionStates(_currentSource, _currentSetting))
                        {
                            ...
                            switch (kv.Value)
                            {
                                ...
                                case string when uiControl is ComboBox box:
                                    box.Text = Conversions.ToString(kv.Value);
                                    break;
                                ...
                                case int:
                                    {
                                        ...
                                        if (combobox.Items.OfType<DropdownPresentationMap>().FirstOrDefault(i => i.Id == Conversions.ToInteger(kv.Value)) is { } matchingItem)
                                        {
                            ...
                            case "decimal":
                                {
                                    ...
                                    {
                                        options.Add(elem.Id, Conversions.ToUInteger(box.Text));
                                    }
                                    else
                                    {
